...
seastar::future<json::json_return_type> run_toppartitions_query(db::toppartitions_query& q, http_context &ctx, bool per_shard, bool legacy_request) {
    ...
    future<> scatter_ret = per_shard ? make_ready_future<>() : q.scatter();

    return scatter_ret.then([&q, &ctx, legacy_request, per_shard] {
        return sleep(q.duration()).then([&q, &ctx, legacy_request, per_shard] {
            return q.gather(ctx.db, per_shard, q.list_size()).then([&q, legacy_request, per_shard] (auto topk_results) {
